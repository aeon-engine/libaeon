...
auto get_source_location(const CXCursor cursor)
{
    ...
    return ast::ast_source_location{to_string(clang_getFileName(file)).str(), line, column};
}
                ...
                [&function](const CXCursor c, [[maybe_unused]] const CXCursor parent)
                {
                    switch (clang_getCursorKind(c))
                    {
                        case CXCursor_ParmDecl:
                        {
                            ...
                            function.add_parameter(to_string(clang_getCursorSpelling(c)),
                                                   to_string(clang_getTypeSpelling(type)));
                        }
                        break;
                        default:
                            break;
                ...
                [&ns](CXCursor c, CXCursor parent)
                {
                    ...
                    switch (clang_getCursorKind(c))
                    {
                        ...
                        case CXCursor_EnumDecl:
                        {
                            ...
                            auto &astenum = *ns.emplace_back(std::make_unique<ast::ast_enum>(
                                to_string(clang_getCursorSpelling(c)), internal::get_linkage_kind(c),
                            ...
                            clang_visit(c,
                                        [&astenum](const CXCursor c, [[maybe_unused]] const CXCursor parent)
                                        {
                                            switch (clang_getCursorKind(c))
                                            {
                                                case CXCursor_AnnotateAttr:
                                                {
                                                    astenum.add_annotation(to_string(clang_getCursorSpelling(c)));
                                                }
                                                break;
                        ...
                        case CXCursor_FieldDecl:
                        {
                            ...
                            ast_entity_visitor(c, *ns.emplace_back(std::make_unique<ast::ast_field>(
                                                      to_string(clang_getCursorSpelling(c)),
                                                      to_string(clang_getTypeSpelling(clang_getCursorType(c))),
                                                      internal::get_access_specifier(c), flags,
                                                      internal::get_linkage_kind(c), std::move(source_location))));
                        }
                        break;
                        ...
                        {
                            auto &ctor = ns
                                             .emplace_back(std::make_unique<ast::ast_constructor>(
                                                 to_string(clang_getCursorSpelling(c)), internal::get_linkage_kind(c),
                        ...
                        {
                            ns.push_back(std::make_unique<ast::ast_destructor>(to_string(clang_getCursorSpelling(c)),
