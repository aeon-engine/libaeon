...
auto xml_node::children(const std::string_view child_name) const -> std::vector<xml_node>
{
...
auto xml_node::child(const std::string_view child_name) const noexcept -> xml_node
{
    if (type_ != xml_node_type::document && type_ != xml_node_type::element)
...
auto xml_node::operator[](const std::string_view child_name) const noexcept -> xml_node
{
    return child(child_name);
}

auto xml_node::attributes() const -> std::map<std::string, variant::converting_variant>
{
    ...
    for (const auto &pt_child : pt_children)
    {
        if (pt_child.is_object())
        {
            ...
            std::map<std::string, variant::converting_variant> attributes;
...
auto xml_node::value_impl() const -> const std::string &
{
